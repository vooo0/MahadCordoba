    ...
    <div class="card">
      <div class="card-body">
        ...
        <table id="dataTableExample2" class="table">
            ...
            <tbody>
            <?php $n1=1;$s=0; ?>
            @foreach($mpGuru as $key => $mpg)
                <tr>
                    <td>{{$n1++}}</td>
                    <td>{{$mpg->matapelajaran->nama}}</td>
                    <td>{{$mpg->tahun->tahun}}</td>
                    <td>{{$getSiswa[$s]}} Siswa</td>
                    <td>{{$mpg->status}}</td>
                    <td>
                      <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#buatGaji{{$mpg->id}}">
                        Tambah Gaji
                      </button>
                      <!-- Modal -->
                      <div class="modal fade" id="buatGaji{{$mpg->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            ...
                            <form class="forms-sample" action="{{route('gaji.gajiGuru', $data->id,'edit')}}" method="post">
                                @csrf
                                <input name="guru_id" type="hidden" class="form-control" id="exampleInputUsername1" autocomplete="off" value="{{$mpg->guru_id}}">
                                ...
                                <div class="modal-body">
                                  <div class="form-group">
                                    ...
                                    <input name="nama" type="text" class="form-control" id="exampleInputUsername1" autocomplete="off" value="Gaji Untuk Kelas <?= $mpg->matapelajaran->nama; ?>">
                                  </div>
                                  <div class="form-group">
        ...
        <table id="dataTableExample" class="table">
            ...
            @foreach($getGaji as $key => $gaji)
                <tr>
                    ...
                    <td>{{$gaji->nama}}</td>
                    <td>Hadir {{$gaji->kehadiran}} Kali</td>
                    ...
                    <td>
                        ...
                        <a href="" class="btn btn-danger btn-icon-text" ><i class="btn-icon-prepend" data-feather="trash"></i>Hapus</a>
                    </td>
                </tr>
                <?php $s++; ?>
            @endforeach
