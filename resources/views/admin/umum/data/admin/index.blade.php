            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form class="forms-sample" action="{{route('dataAdmin.store')}}" method="post">
                    @csrf
                    <div class="form-group">
                      <label>Posisi</label>
                      <select name="posisi" class="form-control form-control-sm mb-3">
            ...
            <tbody>
              ...
              @foreach($data as $key => $admin)
                <tr>
                  ...
                  <td>
                    @if(!empty($admin->telephone))
                    {{$admin->telephone}}
                    @else
                      Belum Diisi
                    @endif
                  </td>
                  <td style="text-align: center;" >{{$admin->posisi}}</td>
                  <td>
                    <form action="{{route('dataAdmin.destroy', $admin->id)}}" method="post">
                      <a href="{{route('dataAdmin.edit', $admin->id)}}" class="btn btn-primary btn-icon-text" ><i class="btn-icon-prepend" data-feather="file"></i>Edit</a>
                      @csrf
                      @method('DELETE')
